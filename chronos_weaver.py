...
class World:
    ...
    if chosen == "A":
        ...
        enemy_paths: List[List[Vec]] = [
            [(4, 1), (5, 1), (6, 1), (7, 1), (7, 2), (7, 3)],
            [(9, 6), (9, 7), (9, 8), (9, 9), (10, 9), (11, 9)],
            [(2, 8), (2, 9), (3, 9), (4, 9), (5, 9), (5, 8)],
        ]
    elif chosen == "C":
        layout = [
            "S....~~....F.C",
        ...
        enemy_paths: List[List[Vec]] = [
            [(0, 5), (1, 5), (2, 5), (3, 5), (4, 5), (5, 5), (6, 5), (5, 5), (4, 5)],
            [(13, 2), (13, 3), (13, 4), (13, 5), (13, 6), (12, 6), (11, 6)],
            [(6, 0), (7, 0), (8, 0), (9, 0), (9, 1), (9, 2), (8, 2)],
        ]
    ...
    def heuristic(self, state: SearchState) -> float:
        ...
        for i, fragment in enumerate(self.fragments):
            if not (state.fragments & (1 << i)):
                todo.append(fragment)
        for i, rift in enumerate(self.rifts):
            if not (state.rifts & (1 << i)):
                todo.append(rift)

        estimate = 0.0
        while todo:
            target = min(todo, key=lambda p: manhattan(current, p))
            estimate += manhattan(current, target)
            current = target
            todo.remove(target)
        return estimate + manhattan(current, self.core)
    ...
    if algorithm == "BFS":
        ...
        while queue and visited < node_limit:
            ...
            if world.is_goal(state):
                ...
                return result(algorithm, True, states, actions, cost, visited, begin, frontier_peak, "Solved")
        ...
        while stack and visited < node_limit:
            ...
            if world.is_goal(state):
                states, actions, cost = reconstruct(state, parent)
                return result(algorithm, True, states, actions, cost, visited, begin, frontier_peak, "Solved")
            for action, child, move_cost in reversed(list(world.neighbors(state))):
                if child in seen:
                    continue
                seen.add(child)
                parent[child] = (state, action, move_cost)
        ...
        while heap and visited < node_limit:
            ...
            for action, child, move_cost in world.neighbors(state):
                ...
                if new_g >= g_score.get(child, float("inf")):
                    continue
                g_score[child] = new_g
                parent[child] = (state, action, move_cost)
                counter += 1
