...
class DuelWorld:
    ...
    def evaluate_enemy(self, state: DuelState) -> float:
        ...
        score = (state.ehp - state.php) * 45
        score += (state.eenergy - state.penergy) * 10
        score -= distance * 5
        if distance <= 1:
            score += 24
        if self.line_of_sight(state.enemy_pos, state.player_pos, 4) and state.eenergy > 0:
            score += 38
        if self.line_of_sight(state.player_pos, state.enemy_pos, 4) and state.penergy > 0:
            score -= 30
        for mine in bit_positions(state.emines):
            score += max(0, 5 - manhattan(state.player_pos, mine)) * 10
            score -= max(0, 3 - manhattan(state.enemy_pos, mine)) * 5
        for mine in bit_positions(state.pmines):
            score -= max(0, 5 - manhattan(state.enemy_pos, mine)) * 10
            score += max(0, 3 - manhattan(state.player_pos, mine)) * 5
...
def choose_ai_action(world: DuelWorld, state: DuelState, algorithm: str, perspective: int = ENEMY) -> AiReport:
    ...
    for action in ordered_actions(world, state):
        ...
        if algorithm == "Minimax":
            ...
            score = alpha_beta(child, depth - 1, -float("inf"), float("inf"))
        if score > best_score:
            best_action = action
            best_score = score
